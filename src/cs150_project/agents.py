...
class UnsophisticatedUltimatumAgent(BaseUltimatumAgent):
    ...
    def choose_split(self)->float:
        ...
        if self.strategy == "random":
            ...
            raise NotImplementedError(f"Strategy {self.strategy} not yet implemented, please use a valid strategy!")
    ...
    def choose_response(self, offer:float)->bool:
        ...
        if self.strategy == "random":
            ...
            return offer > (1-self.fixed_ratio)
...
class OpenAIUltimatumAgent(BaseUltimatumAgent):
    def __init__(self, role:str, strategy:str, system_prompt:str, turn_prompt:str, model:str="gpt-5.4"):

        #in this agent strategy is pretty much just an internal tracker for the prompt versions and such
        super().__init__(role, strategy)
        self.system_prompt = system_prompt ## we need to define the game state, rules etc.
    ...
    def choose_split(self)->float:
        ...
        if self.previous_response_id:
            response = self.client.responses.create(
                        ...
                        response_format=SplitterResponse,
    ...
    def choose_response(self, offer:float)->bool:
        ...
        if self.previous_response_id:
            response = self.client.responses.create(
                model=self.model,
                input=self.turn_prompt,
                response_format=ResponderResponse,
                previous_response_id=self.previous_response_id
            )
    ...
    def generate_freeform_chatter(self, message:str)->str:
        ...
        if self.previous_response_id:
            response = self.client.responses.create(
                model=self.model,
                input=self.turn_prompt,
                response_format=ChatterResponse,
                previous_response_id=self.previous_response_id
            )
    ...
    def accept_freeform_chatter(self, message:str)->None:
