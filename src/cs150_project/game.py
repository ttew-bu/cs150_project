...
class UltimatumGameInstance:
    def __init__(self, splitter:BaseUltimatumAgent,
        ...
        self.results = []
        self.responder_chat_logs = []
        self.splitter_chat_logs = []
    ...
    def play_round(self, round_number:int):
        ...
        if self.comms_allowed:
            ...
            splitter_comment = self.splitter.generate_freeform_chatter(freeform_prompt)
            self.responder.accept_freeform_chatter(hearing_prompt + splitter_comment)

            ## responder needs a prompt addendum:
            responder_freeform_prompt = freeform_prompt

            responder_comment = self.responder.generate_freeform_chatter(responder_freeform_prompt)
            self.splitter.accept_freeform_chatter(hearing_prompt + responder_comment)
        ...
        if self.comms_allowed:
            ...
            responder_comment = self.splitter.generate_freeform_chatter(freeform_prompt)
            self.splitter.accept_freeform_chatter(hearing_prompt + responder_comment)
        ...
        self.results.append({"round":round_number, "split":split, "response":response})
    ...
    def play_game(self):
        ...
        data = {"results":self.results,
                ...
                "responder_chat_logs":self.responder_chat_logs,
                "splitter_chat_logs":self.splitter_chat_logs}
