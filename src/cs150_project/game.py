...
from .constants import FREEFORM_PRETURN_SPEAKER_PROMPT, FREEFORM_PRETURN_LISTENER_PROMPT, \
    FREEFORM_MIDTURN_SPEAKER_PROMPT, FREEFORM_POSTTURN_SPEAKER_PROMPT, FREEFORM_POSTTURN_LISTENER_PROMPT
...
class UltimatumGameInstance:
    ...
    def play_round(self, round_number: int):
        ...
        if self.pregame_comms_allowed:
            splitter_comment = splitter.generate_freeform_chatter(
                FREEFORM_PRETURN_SPEAKER_PROMPT.format(speaker_role=splitter.role), round_chat_logs
            )
            responder.accept_freeform_chatter(
                FREEFORM_PRETURN_LISTENER_PROMPT.format(speaker_role=splitter.role) + splitter_comment, round_chat_logs
            )

            responder_comment = responder.generate_freeform_chatter(
                FREEFORM_PRETURN_SPEAKER_PROMPT.format(speaker_role=responder.role), round_chat_logs
            )
            splitter.accept_freeform_chatter(
                FREEFORM_PRETURN_LISTENER_PROMPT.format(speaker_role=responder.role) + responder_comment, round_chat_logs
            )
        ...
        if self.midgame_comms_allowed:
            responder_comment = splitter.generate_freeform_chatter(
                FREEFORM_MIDTURN_SPEAKER_PROMPT.format(speaker_role=responder.role), round_chat_logs
            )
            splitter.accept_freeform_chatter(
                FREEFORM_MIDTURN_SPEAKER_PROMPT.format(speaker_role=responder.role) + responder_comment, round_chat_logs
            )

            splitter_comment = splitter.generate_freeform_chatter(
                FREEFORM_MIDTURN_SPEAKER_PROMPT.format(speaker_role=splitter.role), round_chat_logs
            )
            responder.accept_freeform_chatter(
                FREEFORM_MIDTURN_SPEAKER_PROMPT.format(speaker_role=splitter.role) + splitter_comment, round_chat_logs
            )
        ...
        if self.postgame_comms_allowed:
            ...
            splitter_comment = splitter.generate_freeform_chatter(
                FREEFORM_POSTTURN_SPEAKER_PROMPT.format(speaker_role=splitter.role,
                                                        split=split,
                                                        verdict_accepted=verdict_accepted_str), round_chat_logs
            )
            responder.accept_freeform_chatter(
                FREEFORM_POSTTURN_LISTENER_PROMPT.format(speaker_role=splitter.role,
                                                         split=split,
                                                         verdict_accepted=verdict_accepted_str
                                                         ) + splitter_comment, round_chat_logs
            )

            responder_comment = splitter.generate_freeform_chatter(
                FREEFORM_POSTTURN_SPEAKER_PROMPT.format(speaker_role=responder.role,
                                                        split=split,
                                                        verdict_accepted=verdict_accepted_str), round_chat_logs
            )
            splitter.accept_freeform_chatter(
                FREEFORM_POSTTURN_LISTENER_PROMPT.format(speaker_role=responder.role,
                                                         split=split,
                                                         verdict_accepted=verdict_accepted_str
                                                         ) + responder_comment, round_chat_logs
            )
    ...
    def play_game(self) -> None | dict[Any, str]:
        ...
        for round_number in range(1, self.rounds + 1):
            ...
            print("result is: " + str(json.dumps(self.results[round_number - 1],indent=4)))
